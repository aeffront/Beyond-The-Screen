  ...
  print(){
    ...
    if (current_prints.length>0){
      for(let i=0;i<current_prints.length;i++){
        instructions.push(`${this.letter} : ${colors[this.pos]} ${shapes[current_prints[i]-1]}`)
        shapeInstructions.push([colors[this.pos],shapes[current_prints[i]-1]])
      }
...
function addShapes(){
  ...
  for(let i=0;i<shapeInstructions.length;i++){
    ...
    img.setAttribute('src', `./shapes/${shapeInstructions[i][0]}/${shapeInstructions[i][1]}.svg`);
